...
#' If operative is not NULL, the following parameters do not need to be specified.
#' @param url ncml directory (FOR DATA LOADING).
#' @param mask Directory (including file name) of the land-mask grid (*.rda) (FOR DATA LOADING).
...
#' Options are: 5:8, 8:11, 11:2 and 2:5.
...
indexOperative <- function(dataset = "CFSv2_seasonal_operative",
      ...
      if(is.null(operative)){
            if(is.null(url)) stop("Please, specify parameter url for data loading.")
            ...
            save(operative, file = paste(destdir, "/", "operative_", years, "_", season[2], "_", season[length(season)], ".rda", sep = ""))
      }else{
            operative <- get(load(operative))
            years <- unique(getYearsAsINDEX(operative))
            leadMonth <- attr(operative, "leadMonth") 
            months <- unique(as.integer(substr(operative$Dates$start, start = 6, stop = 7)))
            season <- c(months[1]-1, months)
      }
      index <- match.arg(index, choices = c("fwi"))
...
wfwiOP <- function(dataset = "CFSv2_seasonal_operative",
      ...
      for(i in 1:(length(x)-1)){
            ...
            dates <- W$Dates
            #             Tm <- transformeR:::redim(Tm)
            #             H <- transformeR:::redim(H)
            #             r <- transformeR:::redim(r)
            #             W <- transformeR:::redim(W)
            #             
            W$Data <- W$Data*3.6
            ...
            xx <- transformeR:::getShape(Tm)[["lat"]]-1
            Tm <- H <- r <- W <- NULL
            if(index == "fwi"){
      ...
      attr(outgrid, "leadMonth") <- leadMonth
